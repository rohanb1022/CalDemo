...
function Footer() {
    const data = {
        facebookLink: 'https://facebook.com', instaLink: 'https://instagram.com', twitterLink: 'https://twitter.com', githubLink: 'https://github.com', dribbbleLink: 'https://dribbble.com',
        services: { webdev: '#', webdesign: '#', marketing: '#', googleads: '#', },
        about: { history: '#', team: '#', handbook: '#', careers: '#', },
        help: { faqs: '#', support: '#', livechat: '#', },
        contact: { email: 'hello@example.com', phone: '+91 1234567890', address: 'Mumbai, Maharashtra, India', },
        company: { name: 'FinHealth', description: 'Empowering you to take control of your financial future with clear, actionable insights.', logo: '/logo.webp', },
    };
    const socialLinks = [ { icon: Facebook, label: 'Facebook', href: data.facebookLink }, { icon: Instagram, label: 'Instagram', href: data.instaLink }, { icon: Twitter, label: 'Twitter', href: data.twitterLink }, { icon: Github, label: 'GitHub', href: data.githubLink }, { icon: Dribbble, label: 'Dribbble', href: data.dribbbleLink }, ];
    const aboutLinks = [ { text: 'Company History', href: data.about.history }, { text: 'Meet the Team', href: data.about.team }, { text: 'Employee Handbook', href: data.about.handbook }, { text: 'Careers', href: data.about.careers }, ];
    const serviceLinks = [ { text: 'Financial Planning', href: data.services.webdev }, { text: 'Investment Advice', href: data.services.webdesign }, { text: 'Tax Strategy', href: data.services.marketing }, { text: 'Retirement Goals', href: data.services.googleads }, ];
    const helpfulLinks = [ { text: 'FAQs', href: data.help.faqs }, { text: 'Support', href: data.help.support }, { text: 'Live Chat', href: data.help.livechat, hasIndicator: true }, ];
    const contactInfo = [ { icon: Mail, text: data.contact.email }, { icon: Phone, text: data.contact.phone }, { icon: MapPin, text: data.contact.address, isAddress: true }, ];

    return (
        <footer className="w-full mt-16 bg-white dark:bg-gray-900">
            <div className="max-w-screen-xl px-4 pt-16 pb-6 mx-auto sm:px-6 lg:px-8 lg:pt-24">
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                    <div>
                        <div className="flex justify-center gap-2 text-primary sm:justify-start">
                            <img src="https://assets.aceternity.com/logo-dark.png" alt="logo" className="w-8 h-8 rounded-full"/>
                            <span className="text-2xl font-semibold">{data.company.name}</span>
                        </div>
                        <p className="max-w-md mt-6 leading-relaxed text-center text-gray-500 sm:max-w-xs sm:text-left dark:text-gray-400">{data.company.description}</p>
                        <ul className="flex justify-center gap-6 mt-8 sm:justify-start md:gap-8">
                            {socialLinks.map(({ icon: Icon, label, href }) => (
                                <li key={label}><Link href={href} className="text-gray-700 transition dark:text-gray-200 hover:text-gray-900 dark:hover:text-white"><span className="sr-only">{label}</span><Icon className="size-6" /></Link></li>
                            ))}
                        </ul>
                    </div>
                    <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 md:grid-cols-4 lg:col-span-2">
                        <div className="text-center sm:text-left"><p className="text-lg font-medium text-gray-900 dark:text-white">About Us</p><ul className="mt-8 space-y-4 text-sm">{aboutLinks.map(({ text, href }) => (<li key={text}><a className="text-gray-700 transition dark:text-gray-400 hover:text-gray-700/75 dark:hover:text-white/75" href={href}>{text}</a></li>))}</ul></div>
                        <div className="text-center sm:text-left"><p className="text-lg font-medium text-gray-900 dark:text-white">Services</p><ul className="mt-8 space-y-4 text-sm">{serviceLinks.map(({ text, href }) => (<li key={text}><a className="text-gray-700 transition dark:text-gray-400 hover:text-gray-700/75 dark:hover:text-white/75" href={href}>{text}</a></li>))}</ul></div>
                        <div className="text-center sm:text-left"><p className="text-lg font-medium text-gray-900 dark:text-white">Helpful Links</p><ul className="mt-8 space-y-4 text-sm">{helpfulLinks.map(({ text, href, hasIndicator }) => (<li key={text}><a href={href} className={hasIndicator ? 'group flex justify-center gap-1.5 sm:justify-start' : 'text-gray-700 dark:text-gray-400 transition hover:text-gray-700/75 dark:hover:text-white/75'}><span className="text-gray-700 transition dark:text-gray-400 hover:text-gray-700/75 dark:hover:text-white/75">{text}</span>{hasIndicator && (<span className="relative flex size-2"><span className="absolute inline-flex w-full h-full rounded-full bg-primary animate-ping opacity-75" /><span className="relative inline-flex rounded-full size-2 bg-primary" /></span>)}</a></li>))}</ul></div>
                        <div className="text-center sm:text-left"><p className="text-lg font-medium text-gray-900 dark:text-white">Contact Us</p><ul className="mt-8 space-y-4 text-sm">{contactInfo.map(({ icon: Icon, text, isAddress }) => (<li key={text}><a className="flex items-center justify-center gap-1.5 sm:justify-start" href="#"><Icon className="size-5 shrink-0 text-primary" />{isAddress ? (<address className="flex-1 not-italic text-gray-700 dark:text-gray-400">{text}</address>) : (<span className="flex-1 text-gray-700 dark:text-gray-400">{text}</span>)}</a></li>))}</ul></div>
                    </div>
                </div>
                <div className="pt-6 mt-12 border-t border-gray-100 dark:border-gray-800"><div className="text-center sm:flex sm:justify-between sm:text-left"><p className="text-sm text-gray-500 dark:text-gray-400"><span className="block sm:inline">All rights reserved.</span></p><p className="mt-4 text-sm text-gray-500 sm:order-first sm:mt-0 dark:text-gray-400">&copy; 2025 {data.company.name}</p></div></div>
            </div>
        </footer>
    );
}
